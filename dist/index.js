...
var appRouter = router({
  ...
  // Lessons Router
  lessons: router({
    getByCourseId: publicProcedure.input(z2.object({ courseId: z2.number() })).query(async ({ input }) => {
  ...
  progress: router({
    getCourseProgress: protectedProcedure.input(z2.object({ courseId: z2.number() })).query(async ({ ctx, input }) => {
      return await getUserProgress(ctx.user.id, input.courseId);
    }),
    markLessonComplete: protectedProcedure.input(z2.object({ courseId: z2.number(), lessonId: z2.number() })).mutation(async ({ ctx, input }) => {
